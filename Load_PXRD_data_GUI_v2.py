...
class MainWindow(QMainWindow):
    ...
    def find_peaks(self,x,y):
        ...
        width=float(self.width.text()) if len(self.width.text())>0 else None
        prominence=float(self.prominence.text()) if len(self.prominence.text())>0 else None
        distance=float(self.distance.text()) if len(self.distance.text())>0 else None
        threshold=float(self.threshold.text()) if len(self.threshold.text())>0 else None
        ...
        theta_scale=len(self.PXRD['2Theta'])/(np.max(self.PXRD['2Theta'])-np.min(self.PXRD['2Theta']))
        peaks, _ = find_peaks(y, prominence=prominence,width=width*theta_scale,distance=distance*theta_scale,threshold=threshold)
        self.peaks={'2Theta':self.PXRD['2Theta'][peaks],'Intensity':self.PXRD['Intensity'][peaks]}
    ...
    def analysis_GUI(self):
        ...
        l5_button.clicked.connect(lambda: self.find_peaks(self.PXRD['2Theta'],self.PXRD['Intensity']))
